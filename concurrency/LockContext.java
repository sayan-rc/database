...
public class LockContext {
    ...
    public void childRemove(TransactionContext transaction, LockContext parent) {
        if (parent == null) {
            return;
        }
        int childLocks = parent.numChildLocks.getOrDefault(transaction.getTransNum(), 0);
        parent.numChildLocks.put(transaction.getTransNum(), childLocks - 1);
        childRemove(transaction, parent.parent);
    }

    public boolean childOf(ResourceName child, ResourceName parent) {
        LockContext parentLockContext = fromResourceName(this.lockman, parent);
        LockContext childLockContext = fromResourceName(this.lockman, child);
        ...
        return childOf(childLockContext.parent.name, parent);
    }

    public void parentAdd(TransactionContext transaction, LockContext parent) {
        if (parent == null) {
            return;
        }
        int parentLocks = parent.numChildLocks.getOrDefault(transaction.getTransNum(), 0);
        parent.numChildLocks.put(transaction.getTransNum(), parentLocks + 1);
        parentAdd(transaction, parent.parent);
    }

    public void upgrade(TransactionContext transaction, LockType locktype, List<ResourceName> releases) {
        this.lockman.acquireAndRelease(transaction, name, locktype, releases);
        parentAdd(transaction, this);
    }

    private void updateRelease(TransactionContext transaction) {
        this.lockman.release(transaction, name);
        childRemove(transaction, parent);
    }
    ...
    throws InvalidLockException, DuplicateLockRequestException {
        ...
        if (this.parent != null && this.parent.saturation(transaction) >= .2 && this.parent.capacity() >= 10) {
        ...
        if (this.parent == null) {
            ...
            lType = parent.getExplicitLockType(transaction);
        }

        if (parent == null || LockType.substitutable(lType, LockType.parentLock(lockType))) {
            this.lockman.acquire(transaction, name, lockType);
            parentAdd(transaction, this.parent);
        } else {
    ...
    throws NoLockHeldException, InvalidLockException {
        ...
        if (!this.numChildLocks.keySet().contains(transaction.getTransNum())) {
            updateRelease(transaction);
    ...
    throws DuplicateLockRequestException, NoLockHeldException, InvalidLockException {
        ...
        LockType lType = getEffectiveLockType(transaction);
        boolean update = lType == LockType.NL ? true : false;
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        if (numChildLocks.getOrDefault(transaction.getTransNum(), 0) == 0 &&
            (getExplicitLockType(transaction) == LockType.X)
        ...
        boolean u = false;
        boolean c = false;
        List<ResourceName> r = new ArrayList<>();

        for (Lock lock : this.lockman.getLocks(transaction)) {
            if (!childOf(lock.name, name)) {
                c = true;
            } else {
                ...
                r.add(lock.name);
                childRemove(transaction, fromResourceName(this.lockman, lock.name));
                c = false;
            }
        }
        ...
        if (u) {
            upgrade(transaction, LockType.X, r);
        } else {
            upgrade(transaction, LockType.S, r);
        }
    }
