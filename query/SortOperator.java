...
public class SortOperator {
    ...
    public Run sortRun(Run run) {
        ...
        Run returnRun = createRun();
        Iterator<Record> runIterator = run.iterator();
        List<Record> runs = new ArrayList<>();
        while (runIterator.hasNext()) {
            runs.add(runIterator.next());
        }
        runs.sort(comparator);
        returnRun.addRecords(runs);
        return returnRun;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        Pair<Record, Integer> recordPair;
        List<Iterator<Record>> recordIterators = new ArrayList<>();
        Queue<Pair<Record, Integer>> q =  new PriorityQueue<>(new RecordPairComparator());
        int index;
        for (int i = 0; i < runs.size(); i++) {
            recordIterators.add(runs.get(i).iterator());
        }
        for (int i = 0; i < runs.size(); i++) {
            q.add(new Pair<>(recordIterators.get(i).next(), i));
        }

        while (q.size() > 0) {
            recordPair = q.remove();
            mergedRun.addRecord(recordPair.getFirst().getValues());
            index = recordPair.getSecond();
            if (recordIterators.get(index).hasNext()) {
                q.add(new Pair<>(recordIterators.get(index).next(), index));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int index = numBuffers - 1;
        int maximum = numBuffers - 1;

        for (int i = 0; i < runs.size(); i += maximum) {
            index = Math.min(runs.size(), i + maximum);
            runsList.add(mergeSortedRuns(runs.subList(i, index)));
        }
    ...
    public String sort() {
        ...
        while (pageIterator.hasNext()) {
            sortedRuns.add(sortRun(createRunFromIterator(
                transaction.getBlockIterator(tableName, pageIterator, numBuffers))));
        }
